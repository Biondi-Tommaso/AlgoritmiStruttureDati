...
// Ritorna una network vuota
  ...
  void scorriAlbero(tree::NodeP root, list::List list){
  	if(root == tree::emptyNode)
		return;
	scorriAlbero(root -> left, list);
	list::addBack( root -> user.usr_Log, list);
	scorriAlbero (root -> right, list);
